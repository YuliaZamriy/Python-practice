...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        # set control variables for more concise coding
        num_letters = len(string.ascii_lowercase)
        alphabet = string.ascii_lowercase
        
        # iterate through 26 lettes of the alphabet to set them as keys 
    ...
    def apply_shift(self, shift):
        ...
        '''
        # initialize encryped message as empty list for mutation
        message_text_list = []
        
        message_text = self.get_message_text() 
        dict52 = self.build_shift_dict(shift)
        
        # encrypt message based on words, and leave non-words as they are
        for i in message_text:
            if i in string.ascii_letters:
                message_text_list.append(dict52[i])
            else:
                message_text_list.append(i)
        
        self.message_text = ''.join(message_text_list)
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        '''
        # initialize counter of valid words
        valid = 0
        
        # iterate through all possible values of shift (1 through 25)
        for s in range(1, 27):
            
            # reset valid words counter for each value of shift
            count = 0
...
#            for word in Message.apply_shift(self, 26 - s).split(' '):
            ...
            # if count of valid words is larger than previous max, 
            # save best_shift and decrypted message
            if count > valid:
                valid = count
